  ...
  if anySFNs == "flat"
    then do 
      ...
      if howMany < 0
        then do 
          ...
          putStrLn(letter++anySFNs++" Major has "++toString++" flats. The flats are:")
          print(yourFlats)
    ...
    then do 
      ...
      then do 
        ...
        putStrLn(letter++anySFNs++" Major has "++toString++" sharp(s). The sharp(s) is(are):")
        print(yourSharps)
    ...
    then do
      ...
      if howMany < 0
        then do 
          ...
          let yourFlats = take (abs(howMany)) (cycle orderOfFlats)
          let toString = show(length (yourFlats))
          putStrLn(letter++anySFNs++" Major has "++toString++" flat(s). The flat(s) is(are):")
          print(yourFlats)
      else if howMany > 0 
        then do 
          let yourSharps = take howMany (cycle orderOfSharps)
          let toString = show(length (yourSharps))
          putStrLn(letter++anySFNs++" Major has "++toString++" sharp(s). The sharps is(are):")
          print(yourSharps)
      else do
          print("CN Major has no sharps/flats")
  else do
